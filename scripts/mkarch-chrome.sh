...
export MACHINE_NAME="${MACHINE_NAME:-chrome}"
...
# sudo paru --root "$MACHINE_DIR" --cachedir /var/cache/pacman/pkg -S google-chrome-dev
# paru --sysroot "$MACHINE_DIR" --cachedir /var/cache/pacman/pkg -S google-chrome-dev
...
[Unit]
Description=Google Chrome Dev Browser
...
ExecStart=/usr/bin/google-chrome-unstable --ozone-platform-hint=wayland --ozone-platform=wayland --enable-features=UseOzonePlatform,WaylandWindowDecorations --enable-wayland-ime --wayland-text-input-version=3 $@
...
paru -S --root "$MACHINE_DIR" google-chrome-dev
